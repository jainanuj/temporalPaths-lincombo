...
void Graph::transform(){
   ...
   /*cout << "|Vin| for source: " << Vin[24979].size() << endl;
   cout << "|Vout| for source: " << Vout[24979].size() << endl;
   cout << "|Vin| for target: " << Vin[1162].size() << endl;
   cout << "|Vout| for target: " << Vout[1162].size() << endl;
   cout << "Vin set for target:" << endl;
   for(auto setIt = Vin[1162].begin(); setIt != Vin[1162].end(); setIt++){
	int newIndex = *setIt;
	cout << node_list[newIndex].t << endl;
   }
   cout << "end of the Vin list" << endl;*/

   cout << "Vin set for the source:" << endl;
   for(auto setIt = Vin[24979].begin(); setIt != Vin[24979].end(); setIt++){
        int newIndex = *setIt;
        cout << node_list[newIndex].t << endl;
   }
   cout << "end of the Vin list" << endl;
   ...
   /*int source = 24979;
   int target = 1162;
   int tID = inMap[make_pair(target, 1019196)];
   for(auto tIt = Tout[source].begin(); tIt != Tout[source].end(); tIt++){
	int sID = outMap[make_pair(source, *tIt )];
	bool res = DFS(sID, tID, sID);
	if(res){
           cout << "path complete! source: " << source << endl;
	   return;
        }
   }
   cout << "no path found!!!!" << endl;*/

   int from_id = outMap[make_pair(24979, 1019195)];
   int to_id = inMap[make_pair(1162, 1019196)];
   for(auto it2 = adj_list[from_id].begin(); it2 != adj_list[from_id].end(); it2++){
	if(it2->first == to_id){
	   cout << "the link exists" << endl;
	   return;
	}
   }
   cout << "the link doesn't exist" << endl;

}
bool Graph::DFS(int sID, int tID, int cur){
   if(cur == tID){
      if(cur == sID)
	cout << "(source) ";
      cout << "node: " << node_list[tID].u << ", t: " << node_list[tID].t << ", ";
      if(node_list[tID].isVin)
	 cout << "is Vin" << endl;
      else 
	 cout << "is Vout" << endl;
      return true;
   ...
   for(auto it=adj_list[cur].begin(); it != adj_list[cur].end(); it++){
	bool res = DFS(sID, tID, it->first);
	if(res){
           cout << "node: " << node_list[cur].u << ", t: " << node_list[cur].t << ", ";
           if(node_list[cur].isVin)
              cout << "is Vin" << endl;
           else
              cout << "is Vout" << endl;
           return true;
